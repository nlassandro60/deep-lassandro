...
from spare.sae_lens.config import DTYPE_MAP, LanguageModelSAERunnerConfig
from spare.sae_lens.sae import SAE, SAEConfig
from spare.sae_lens.toolkit.pretrained_sae_loaders import (
...
class TrainStepOutput:
    sae_in: torch.Tensor = None
    sae_out: torch.Tensor = None
    feature_acts: torch.Tensor = None
    loss: torch.Tensor = None  # we need to call backwards on this
    mse_loss: float = None
    l1_loss: float = None
    ghost_grad_loss: float = None
...
class TrainingSAEConfig(SAEConfig):
    ...
    lp_norm: float = None
    use_ghost_grads: bool = None
    normalize_sae_decoder: bool = None
    noise_scale: float = None
    decoder_orthogonal_init: bool = None
